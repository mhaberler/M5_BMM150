...
int8_t i2c_write(uint8_t dev_id, uint8_t reg_addr, uint8_t *read_data,
                 uint16_t len) {
  if (m5::Ex_I2C.write(
          dev_id, reg_addr, read_data,
          len)) { // Writes data of length len to the specified device address.
...
void setup() {
  ...
  img.setTextColor(TFT_WHITE); // Set the font foreground colour (background is.
                               // 设置字体的前景色为TFT_WHITE
  img.createSprite(320, 240);  // Create a sprite (bitmap) of defined width and
                              // height 创建一个指定宽度和高度的Sprite图
  ...
  if (bmm150_initialization() != BMM150_OK) {
    ...
    img.pushSprite(
        0, 0); // Push the sprite to the TFT at 0, 0.  将Sprite图打印在(0,0)处
    for (;;) {
...
void loop() {
  ...
  float head_dir = atan2(dev.data.x - mag_offset.x, dev.data.y - mag_offset.y) *
                   180.0 / M_PI;
